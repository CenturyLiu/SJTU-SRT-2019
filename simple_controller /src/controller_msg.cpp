...
void Callback_control(const simple_controller::Srt_Control &msg){
    ...
    unsigned un_angle;//from 9,179(right,2483) to 228,157(left, 58525).
	int angle;
    unsigned angle_high;
    unsigned angle_low;
    ...
    angle =  slope * obj_ang + intercept;
	if (angle >= 0) un_angle = angle;
	else un_angle = 0;
    if(un_angle > 65535) un_angle = 65535;
    else un_angle = un_angle;

    angle_high = un_angle >> 8;
    angle_low = un_angle - (angle_high << 8);
    VCI_CAN_OBJ vco[1];
    vco[0].ID = 0xa3;
    vco[0].SendType = 0;// 正常发送
    vco[0].RemoteFlag = 0;// 数据帧
